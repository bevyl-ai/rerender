...
function resolveTrack(moovBytes: Uint8Array): ResolvedTrack {
  ...
  const resolution = ((): CodecResolution => {
    ...
    if (!handler) {
      return { ok: false, reason: 'unsupported-codec', sampleEntry: claimed.entry.type };
    }
...
export function parseSampleTable(moovBytes: Uint8Array): SampleTable {
  ...
  if (!track) throw new ExtractError('no-video-track', 'mp4 sample table: no video track');
  ...
  const ctts = find('ctts');
  {
    ...
    if (ctts) {
      ...
      for (let i = 0; i < entryCount && sample < sampleCount; i++) {
        ...
        for (let j = 0; j < count && sample < sampleCount; j++, sample++) {
          const ticks = decodeTicks[sample];
          if (ticks === undefined) {
            throw new ExtractError('malformed', `mp4 sample table: stts left sample ${sample} without a decode timestamp`);
          }
          presentationTicks[sample] = ticks + offset - editShift;
        }
      }
    }
    for (; sample < sampleCount; sample++) {
      ...
      presentationTicks[sample] = ticks - editShift;
    }
  }
  ...
  const byteOffsets = new Float64Array(sampleCount);
  {
    ...
    for (let chunk = 0; chunk < chunkCount && sample < sampleCount; chunk++) {
      ...
      for (let i = 0; i < samplesPerChunk && sample < sampleCount; i++, sample++) {
        ...
        const size = byteSizes[sample];
        if (size === undefined) {
          throw new ExtractError('malformed', `mp4 sample table: stsz left sample ${sample} without a size`);
        }
        offset += size;
      }
    }
