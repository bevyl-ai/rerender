...
function FadePresentation({
  ...
  const opacity =
    presentationDirection === 'entering' ? presentationProgress : passedProps.shouldFadeOutExitingScene ? 1 - presentationProgress : 1;
  return <AbsoluteFill style={{ opacity }}>{children}</AbsoluteFill>;
}

export function fade({ shouldFadeOutExitingScene = false }: { shouldFadeOutExitingScene?: boolean } = {}): TransitionPresentation<unknown> {
