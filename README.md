...
specifier in your own imports, alias it: `npm i rerender@npm:@bevyl-ai/rerender`.
...
## `@bevyl-ai/rerender/extract` — any frame of any mp4, in milliseconds
...
| scenario | `@bevyl-ai/rerender/extract` | `@remotion/webcodecs` |
| -- | -- | -- |
...
| `@bevyl-ai/rerender/extract` | zero-dependency mp4 frame extraction + frame store |
| `@bevyl-ai/rerender/media-parser` | `@remotion/media-parser`'s `parseMedia`, over mediabunny |
...
Requires Node >=20.3 for the CLI; `react`/`react-dom` >=18 are peer dependencies. `@bevyl-ai/rerender/extract`
